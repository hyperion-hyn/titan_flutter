...
import 'package:titan/src/pages/node/widget/node_join_member_widget_v8.dart';
...
class Map3NodeContractDetailPageV8 extends StatefulWidget {
  final int contractId;

  Map3NodeContractDetailPageV8(this.contractId);

  @override
  _Map3NodeContractDetailStateV8 createState() => new _Map3NodeContractDetailStateV8();
}

class _Map3NodeContractDetailStateV8 extends BaseState<Map3NodeContractDetailPageV8> {
    ...
    switch (_contractState) {
      ...
      case ContractState.ACTIVE:
        ...
        _contractStateDetail =
            FormatUtil.timeStringSimpleV8(context, _contractNodeItem.completeSecondsLeft.toDouble()) + suffix;
...
//        _userDelegateState?.index<UserDelegateState?.ACTIVE?.index? 0:_contractNodeItem.instanceFinishTime,
      ...
      progressHints = [
        S.of(context).n_day_v8(7.toString()),
        S.of(context).n_day_v8(90.toString()),
        S.of(context).n_day_v8(90.toString()),
        "",
        ""
      ];
      ...
      progressHints = [
        S.of(context).n_day_v8(7.toString()),
        S.of(context).n_day_v8(_contractNodeItem.contract.duration.toString()),
        "",
        ""
      ];
    }
  ...
  Future getContractDetailData() async {
    try {
      ...
      if (_isNoWallet) {
        ...
        _isDelegated = await _api.checkIsDelegatedContractInstanceV8(widget.contractId);
        if (_isDelegated) {
          _contractDetailItem = await _api.getContractDetailV8(widget.contractId);
...
//        );
  ...
  void _pushWalletManagerAction() {
    Application.router.navigateTo(context,
        Routes.map3node_create_wallet + "?pageType=${Map3NodeCreateWalletPageV8.CREATE_WALLET_PAGE_TYPE_JOIN}");
  }
  ...
  void _broadcaseContractAction() async {
    ...
    await Application.router.navigateTo(
        context,
        Routes.map3node_broadcase_success_page_v8 +
