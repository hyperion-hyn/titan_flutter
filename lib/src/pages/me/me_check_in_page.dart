...
class _MeCheckIn extends BaseState<MeCheckIn> {
  ...
  void onCreated() {
    ...
    //根据算力计算扫描范围
    if (userInfo.totalPower <= 1) {
      minZoom = 17;
      maxMeter = 30;
    } else if (userInfo.totalPower <= 10) {
      minZoom = 17;
      maxMeter = 100;
    } else if (userInfo.totalPower <= 20) {
      minZoom = 16;
      maxMeter = 500;
    } else if (userInfo.totalPower <= 50) {
      minZoom = 15;
      maxMeter = 1200;
    } else if (userInfo.totalPower <= 100) {
      minZoom = 14;
      maxMeter = 2000;
    } else if (userInfo.totalPower <= 500) {
      minZoom = 13;
      maxMeter = 5000;
    } else {
      minZoom = 12;
            ...
            child: Container(
              child: Column(
                ...
                children: <Widget>[
                  Container(
                    child: Text(
                      S.of(context).all_powers_func('${MeUtils.powerForShow(userInfo?.totalPower)}'),
                      style: TextStyle(color: Colors.white),
                    ),
