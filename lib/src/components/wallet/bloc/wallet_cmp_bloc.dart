...
//            var q = symbolQuoteEntity.SymbolQuoteVo.clone(quote);
        ...
        if (event.updateGasPrice) {
          BlocProvider.of<WalletCmpBloc>(Keys.rootKey.currentContext)
      ...
      try {
        ...
        await AppCache.saveValue(PrefsKey.SHARED_PREF_GAS_PRICE_KEY,
            json.encode(gasPriceRecommend.toJson()));
        yield GasPriceState(
            status: Status.success, gasPriceRecommend: gasPriceRecommend);
        isGasSuccess = true;
        ...
        if (btcResponse["code"] == 0) {
          ...
          await AppCache.saveValue(PrefsKey.SHARED_PREF_BTC_GAS_PRICE_KEY,
              json.encode(btcGasPriceRecommend.toJson()));
          yield GasPriceState(
        ...
        if (!isGasSuccess) {
          ...
          if (gasPriceEntityStr != null) {
            yield GasPriceState(
                status: Status.success,
                gasPriceRecommend: json.decode(gasPriceEntityStr));
          } else {
      ...
      for (var cVo in vo.coins) {
        for (var dVO in deList) {
          if (cVo.symbol == dVO.symbol &&
              cVo.contractAddress == dVO.contractAddress) {
  ...
  Future requestGasPrice() async {
    ...
    // fastest
    var fastGasPrice =
        double.parse(responseFromEtherScanDict["FastGasPrice"]) * 10.0;
    var fastest =
        double.parse(responseFromEthGasStationDict["fastest"].toString());
    responseFromEthGasStationDict["fastest"] = max(fastGasPrice, fastest);

    // fast
    var proposeGasPrice =
        double.parse(responseFromEtherScanDict["ProposeGasPrice"]) * 10.0;
    ...
    // average
    var safeGasPrice =
        double.parse(responseFromEtherScanDict["SafeGasPrice"]) * 10.0;
    var average =
        double.parse(responseFromEthGasStationDict["average"].toString());
  ...
  Future requestGasFromEthGasStation() async {
    var response = await HttpCore.instance
  ...
  Decimal parseGasPriceToBigIntWei(double num) {
    return Decimal.parse(num.toString()) /
        Decimal.fromInt(10) *
        Decimal.fromInt(TokenUnit.G_WEI);
  }
}
