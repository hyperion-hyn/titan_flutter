...
class _SeekBarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 画当前显示的值的指示器
    void drawIndicator() async{
      Paint indicatorPaint = new Paint()
        ..style = PaintingStyle.fill
        ..color = indicatorColor;
      if(indicatorImg != null){
        Paint indicatorPaint = new Paint();
        var data = await rootBundle.load(indicatorImg);
        var codec = await ui.instantiateImageCodec(data.buffer.asUint8List());
        var info = await codec.getNextFrame();
        canvas.drawImage(info.image, Offset(value * size.width, size.height / 2), indicatorPaint);
      } else {
...
@immutable
class CustomSeekBar extends BasicSeekbar {
  ///显示气泡 ,默认是隐藏的，true
  bool hideBubble;
...
class _SeekBarState extends State<CustomSeekBar> {
  ///进度值
  double _value;
      ...
      child: Container(
        ...
        child: CustomPaint(
          painter: _SeekBarPainter(
            ...
            indicatorImg: widget.indicatorImg,
          ),
        ),
      ),
    );
  ...
  @override
  void didUpdateWidget(CustomSeekBar oldWidget) {
