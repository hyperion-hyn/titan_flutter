...
//    }).toList();
  ...
  static web3.DeployedContract _newRpHoldingContract(String contractAddress) {
    final contract = web3.DeployedContract(
        web3.ContractAbi.fromJson(RP_HOLDING_ABI, 'RpHolding'),
        web3.EthereumAddress.fromHex(contractAddress));
    return contract;
  }
  ...
  static Future<dynamic> postToEthereumNetwork(
    ...
    return HttpCore.instance.post(
        isAtlasTrans
            ? WalletConfig.getAtlasApi()
            : WalletConfig.getEthereumApi(),
        params: {
          "jsonrpc": "2.0",
          "method": method,
          "params": params,
  ...
  static web3.DeployedContract getRpHoldingContract(String contractAddress) {
    if (_rpHoldingContract == null ||
        _rpHoldingContract?.address?.hex?.contains(contractAddress) != true) {
  ...
  static web3.Web3Client getWeb3Client(
      [bool isAtlas = false, bool isPrint = false]) {
    if (isAtlas) {
